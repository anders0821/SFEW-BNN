...
def trial(N_HIDDEN_LAYERS, NUM_UNITS, OUTPUT_TYPE, MAIN_LOSS_TYPE, LAMBDA, FOLD, FINTUNE_SNAPSHOT, FINTUNE_SCALE):
    ...
    mlp = lasagne.layers.DropoutLayer(
            mlp, 
            p=0)# no dropout on input & BAE-1 & BAE-2 layer

    '''
    hidden layers
    k=0 MDLBP-1500 BAE1
    k=1 1500-1500 BAE2
    k=2 1500-7 (only N_HIDDEN_LAYERS >=3 )
    '''
    for k in range(N_HIDDEN_LAYERS):
        ...
        if (k == 1):
            if (FINTUNE_SNAPSHOT != 0):
                print('Load ./W-BAE2-15001500-%d.npz' % FINTUNE_SNAPSHOT)
                with np.load('./W-BAE2-15001500-%d.npz' % FINTUNE_SNAPSHOT) as f:
                    param_values = [f['arr_%d' % i] for i in range(len(f.files))]
                param_values = param_values[0:8] # (W b bn bn bn bn) (W b) | (W b bn bn bn bn) = 14
                lasagne.layers.set_all_param_values(mlp, param_values)

        # cat k=0 k=1
        if(k==0):
            mlp_cat = mlp
            print('backup mlp_cat')
        if (k == 1):
            mlp = lasagne.layers.concat([mlp, mlp_cat])
            print('cat mlp_cat')
    ...
    if binary:
        ...
        # untrainable W1 W2
        assert(len(W)==N_HIDDEN_LAYERS+1)
        assert(len(W_grads)==N_HIDDEN_LAYERS+1)
        W = W[2:len(W)]
        W_grads = W_grads[2:len(W_grads)]
        assert(len(W)==N_HIDDEN_LAYERS+1-2)
        assert(len(W_grads)==N_HIDDEN_LAYERS+1-2)
        ...
        # untrainable b1 bn1 b2
        assert(len(params)==3+1+(N_HIDDEN_LAYERS+1-2)*3) # 3+1+3+3+3 ...
        ...
        params = params[4:len(params)]
        assert(len(params)==(N_HIDDEN_LAYERS+1-2)*3)
