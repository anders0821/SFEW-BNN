...
def clipping_scaling(updates,network):
    ...
    for layer in layers:
        ...
        params = layer.get_params(binary=True)
        for param in params:
            if(param in updates):# W1 is not in updates
...
def train(train_fn,val_fn,
    ...
    def train_epoch(X,LR):
        ...
        output = []
        output_mlp_groundtruth = []
        output_mlp_midactivation = []

        for i in range(batches):
            #A = lasagne.layers.get_all_param_values(mlp)# check grad update W1(fixed) b1(fixed) beta(fixed) gamma(fixed) mean inv_std W2 b2 W3 b3 beta gamma mean inv_std
            new_loss, new_penalty, new_output_mlp_groundtruth, new_output_mlp_midactivation, new_output = train_fn(X[i * batch_size:(i + 1) * batch_size], LR)
            loss += new_loss*batch_size
            penalty += new_penalty*batch_size
            output.append(new_output)
            output_mlp_groundtruth.append(new_output_mlp_groundtruth)
            output_mlp_midactivation.append(new_output_mlp_midactivation)

        # patch the final partial minibatch
        if len(X) > batches*batch_size:
            new_loss, new_penalty, new_output_mlp_groundtruth, new_output_mlp_midactivation, new_output = train_fn(X[batches*batch_size:len(X)], LR)
        ...
        output_mlp_groundtruth = np.concatenate(output_mlp_groundtruth)
        output_mlp_midactivation = np.concatenate(output_mlp_midactivation)
        sparsity1 = (output_mlp_groundtruth>0).mean()
        sparsity2 = (output_mlp_midactivation>0).mean()
        sparsity3 = (output>0).mean()
        l1 = output_mlp_midactivation.mean()
        print "layer sparsity / l1:", sparsity1, sparsity2, sparsity3, l1
        ...
        output *= 2
        output -= 1  # BTU
        s00 = np.logical_and(output_mlp_groundtruth == -1, output == -1)
        s01 = np.logical_and(output_mlp_groundtruth == -1, output == 1)
        s10 = np.logical_and(output_mlp_groundtruth == 1, output == -1)
        s11 = np.logical_and(output_mlp_groundtruth == 1, output == 1)
        ...
        s11 = np.sum(s11)
        s = s00 + s01 + s10 + s11
        assert (s == output_mlp_groundtruth.shape[0] * output_mlp_groundtruth.shape[1])
    ...
    def val_epoch(X):
        ...
        for i in range(batches):
            #A = lasagne.layers.get_all_param_values(mlp)# check grad update W1 b1 beta gamma mean(update) inv_std(update) W2 b2 W3 b3 beta gamma mean(update) inv_std(update)
            new_loss, new_penalty, new_output_mlp_groundtruth, new_output_mlp_midactivation, new_output = val_fn(X[i*batch_size:(i+1)*batch_size])
            loss += new_loss*batch_size
            penalty += new_penalty*batch_size
            output.append(new_output)
            output_mlp_groundtruth.append(new_output_mlp_groundtruth)
        ...
        # patch the final partial minibatch
        if len(X) > batches*batch_size:
            new_loss, new_penalty, new_output_mlp_groundtruth, new_output_mlp_midactivation, new_output = val_fn(X[batches*batch_size:len(X)])
            loss += new_loss * (len(X) - batches*batch_size)
            penalty +=  new_penalty * (len(X) - batches*batch_size)
            output.append(new_output)
            output_mlp_groundtruth.append(new_output_mlp_groundtruth)
