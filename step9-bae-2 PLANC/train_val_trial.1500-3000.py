# TODO:
# train BAE-2: no dropout on input & BAE-1 layer, and W1 is fixed
# but for high level lasagne limitation, 1st BatchNorm and b1 are still trainable
# BAE-2 will be jointly trained with fixed W1 (most of the knowledges) and 2 dropouts=0 (reconstruct full data each time).
...
def trial(N_HIDDEN_LAYERS, NUM_UNITS, OUTPUT_TYPE, MAIN_LOSS_TYPE, LAMBDA, FOLD):
    # BN parameters
    batch_size = 100
    ...
    mlp = lasagne.layers.DropoutLayer(
            mlp, 
            p=0)# train BAE-2: no dropout on input & BAE-1 layer
    ...
    if MAIN_LOSS_TYPE=='SH':
        ...
        assert(False)
    '''
    [train_output_mlp_groundtruth, train_output_mlp_midactivation, train_output] = lasagne.layers.get_output([mlp_groundtruth, mlp_midactivation, mlp], deterministic=False)
    train_loss = T.mean(T.maximum(0.,1.-train_output_mlp_groundtruth*train_output))
    ...
    '''
    if LAMBDA>0:
        train_penalty = LAMBDA * T.mean(T.maximum(0., 1. + train_output_mlp_midactivation))
    else:
        train_penalty = T.constant(0.)
    train_loss = train_loss + train_penalty
    ...
    '''
    [val_output_mlp_groundtruth, val_output_mlp_midactivation, val_output] = lasagne.layers.get_output([mlp_groundtruth, mlp_midactivation, mlp], deterministic=True)
    val_loss = T.mean(T.maximum(0.,1.-val_output_mlp_groundtruth*val_output))
    ...
    '''
    if LAMBDA>0:
        val_penalty = LAMBDA * T.mean(T.maximum(0., 1. + val_output_mlp_midactivation))
    else:
        val_penalty = T.constant(0.)
    val_loss = val_loss + val_penalty
    ...
    train_fn = theano.function([input, LR], [train_loss, train_penalty, train_output_mlp_groundtruth, train_output_mlp_midactivation, train_output], updates=updates)

    ##############################################################################################

    # Compile a second function computing the validation loss and accuracy:
    val_fn = theano.function([input], [val_loss, val_penalty, val_output_mlp_groundtruth, val_output_mlp_midactivation, val_output])
    ...
    print('Save W')
    np.savez('./W.npz', *lasagne.layers.get_all_param_values(mlp))# W b BN BN BN BN W b BN BN BN BN
...
if __name__ == "__main__":
    ...
    print(N_HIDDEN_LAYERS, NUM_UNITS, OUTPUT_TYPE, MAIN_LOSS_TYPE, LAMBDA, FOLD)
    print('TRIAL BEGIN')
    trial(N_HIDDEN_LAYERS, NUM_UNITS, OUTPUT_TYPE, MAIN_LOSS_TYPE, LAMBDA, FOLD)
    print('TRIAL END')
