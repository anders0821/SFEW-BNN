...
for N_HIDDEN_LAYERS in {1..1}
do
for NUM_UNITS in 1500
do
for OUTPUT_TYPE in C
do
for MAIN_LOSS_TYPE in W1H
do
for LAMBDA in 0
do
for FOLD in {1..1}
...
then
  ...
  echo $fn
  STARTTIME=$(date +%s)
  PATH=/usr/local/cuda-7.5/bin:$PATH LD_LIBRARY_PATH=/usr/local/cuda-7.5/lib64:$LD_LIBRARY_PATH THEANO_FLAGS=floatX=float32,device=gpu python -u ./train_val_trial.py $N_HIDDEN_LAYERS $NUM_UNITS $OUTPUT_TYPE $MAIN_LOSS_TYPE $LAMBDA $FOLD > $fn 2>&1
